...
export class AddEventComponent implements OnInit{
  ...
  constructor(private _AddNewEventService:AddNewEventService , private  _AuthService:AuthService){}
 ...
 submitDataImage(){
   ...
   const userToken = localStorage.getItem('userToken')
     if (userToken) {
       this._AddNewEventService.Upload_image(  formData ,  userToken).subscribe({
         next:(res)=>{
           // console.log(res);
    ...
    if (this.AddEventForm.valid) {
      ...
      if (userToken) {
        const formData = this.AddEventForm.value;
          ...
          // Add new category
          this._AddNewEventService.Add_event(formData, userToken).subscribe({
            next: () => {
