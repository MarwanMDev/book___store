...
export class NewPublisherComponent implements OnInit{
  ...
  constructor(private _AddPublisherService:AddPublisherService , private _AuthService:AuthService , private _UsersSerService:UsersSerService){}
 ...
 submitDataImage(){
   ...
   const userToken = localStorage.getItem('userToken')
     if (userToken) {
       this._AddPublisherService.Upload_image(  formData ,  userToken).subscribe({
         next:(res)=>{
           console.log(res);
  ...
  AddPublisher: FormGroup = new FormGroup({
    ...
    name: new FormControl(null ,[Validators.required , Validators.minLength(2)  , Validators.maxLength(64)]),
    ...
    if (this.AddPublisher.valid) {
      ...
      if (userToken) {
        ...
        const formData = this.AddPublisher.value;
          // Add new category
          this._AddPublisherService.Add_Publisher(formData, userToken).subscribe({
            next: () => {
