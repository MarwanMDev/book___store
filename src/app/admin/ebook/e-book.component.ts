...
export class EBookComponent implements OnInit {
  ...
  constructor(private _AddNewBookService : AddNewBookService , private _AuthService:AuthService , private _AddCatService:AddCatService){}
  ...
  submitDataImage(){
    ...
    const userToken = localStorage.getItem('userToken')
      if (userToken) {
        this._AddNewBookService.Upload_image(  formData ,  userToken).subscribe({
          next:(res)=>{
            // console.log(res);
...
// get file that added
  ...
  submitData(){
    ...
    const userToken = localStorage.getItem('userToken')
      if (userToken) {
        this._AddNewBookService.Upload_pfd(  formData ,  userToken).subscribe({
          next:(res)=>{
            // console.log(res);
  ...
  AddBook: FormGroup = new FormGroup({
    bookName: new FormControl(null , [Validators.required , Validators.minLength(3)  , Validators.maxLength(100) ]),
    authorName: new FormControl(null , [Validators.required ,Validators.minLength(2)  , Validators.maxLength(64) ]),
    price: new FormControl(null , [Validators.required ]),
    delivaryPrice: new FormControl(null ),
    publisherName: new FormControl(null ,[Validators.required , Validators.minLength(2)  , Validators.maxLength(64)]),
    ...
    if (this.AddBook.valid) {
      ...
      if (userToken) {
        const formData = this.AddBook.value;
          ...
          // Add new category
          this._AddNewBookService.Add_book(formData, userToken).subscribe({
            next: (response) => {
